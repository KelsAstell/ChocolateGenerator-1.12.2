package wolf.astell.choco.PoweredByChoco.tile.base;
...
import wolf.astell.choco.PoweredByChoco.energy.capacitor.FEConnection;
...
public class StorageTE extends TileEntity implements ITickable {
	...
	public EnumMap<EnumFacing,Boolean> facingBooleanEnumMap;

	private EnumMap<EnumFacing, FEConnection> fec;

	public StorageTE(StorageTier storageTier) {
		tier = storageTier;
		facingBooleanEnumMap = new EnumMap<EnumFacing,Boolean>(EnumFacing.class);
		fec = new EnumMap<EnumFacing, FEConnection>(EnumFacing.class);
		for(EnumFacing dir : EnumFacing.values()) {
			fec.put(dir, new FEConnection(this, true, true, dir));
		}
	}
	...
	public void update() {
		if (world.getWorldTime() % 400 == 0){
			if (ModConfig.CAPACITOR_CONF.DO_EXPLODE && this.getWorld().canSeeSky(this.getPos()) && this.getWorld().getLight(this.getPos())>=11){//1000%!
				if (world.getWorldTime() % 400 == 0){
					this.getWorld().createExplosion(null, this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), 1.8F, true);
				}
	...
	private void handleSendingEnergy() {
		if (!world.isRemote) {
			...
			for (EnumFacing dir : EnumFacing.values()) {
				if (!facingBooleanEnumMap.containsKey(dir))
					facingBooleanEnumMap.put(dir, false);

				if (facingBooleanEnumMap.get(dir)) {
					facingBooleanEnumMap.put(dir, false);
					continue;
				}
				...
				if (tile != null) {
					if (tile instanceof StorageTE) {
						...
						energy -= difference;
						if (difference > 0) {
							facingBooleanEnumMap.put(dir, true);
						}
					}
					else {
